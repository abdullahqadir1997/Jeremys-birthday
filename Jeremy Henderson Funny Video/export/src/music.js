...
(function () {
  ...
  const MELODY = [
    ...
    // Warning (37-41s)
    [37.0, NOTE.E4, 0.3, 0.3], [37.4, NOTE.F4, 0.3, 0.3], [37.8, NOTE.E4, 0.3, 0.3],
    [38.2, NOTE.F4, 0.3, 0.3], [38.7, NOTE.G4, 0.2, 0.3], [38.95, NOTE.A4, 0.2, 0.3],
    [39.2, NOTE.B4, 0.2, 0.3], [39.45, NOTE.C5, 0.2, 0.3], [39.7, NOTE.D5, 0.2, 0.3],
    [39.95, NOTE.E5, 0.4, 0.35], [40.4, NOTE.G5, 0.5, 0.4],

    // Finale (41-47s)
    [41.0, NOTE.C5, 0.3, 0.4], [41.35, NOTE.E5, 0.3, 0.4], [41.7, NOTE.G5, 0.3, 0.4],
    [42.05, NOTE.C5, 0.5, 0.45], [42.6, NOTE.E5, 0.3, 0.4], [42.95, NOTE.G5, 0.3, 0.4],
    [43.3, NOTE.C5, 0.5, 0.45], [43.9, NOTE.G5, 0.25, 0.4], [44.2, NOTE.A5, 0.25, 0.4],
    [44.5, NOTE.G5, 0.25, 0.4], [44.8, NOTE.F5, 0.25, 0.4], [45.1, NOTE.E5, 0.3, 0.4],
    [45.45, NOTE.D5, 0.3, 0.4], [45.8, NOTE.C5, 1.2, 0.5],
  ];

  const BASS = [];
  // Generate walking bass from 3s to 41s on every half-beat
  for (let t = 3.0; t < 41.0; t += 0.5) {
    const idx = Math.floor((t - 3.0) * 2);
    const root = [NOTE.C4, NOTE.C4, NOTE.F4, NOTE.G4][idx % 4] / 2;
    BASS.push([t, root, 0.45, 0.3]);
  }
  // Finale bass (41-47s)
  for (let t = 41.0; t < 46.5; t += 0.5) {
    const idx = Math.floor((t - 41.0) * 2);
    const root = [NOTE.C4, NOTE.G4, NOTE.C4, NOTE.E4, NOTE.F4, NOTE.G4][idx % 6] / 2;
    BASS.push([t, root, 0.45, 0.35]);
  }
  BASS.push([46.5, NOTE.C4 / 2, 0.8, 0.4]);

  // Kicks every half beat through the song
  const KICKS = [];
  for (let t = 3.0; t < 46.5; t += 0.5) KICKS.push(t);
  KICKS.push(46.5, 47.0);

  // Snares on 2 and 4
  const SNARES = [];
  for (let t = 3.5; t < 46.0; t += 1.0) SNARES.push(t);
